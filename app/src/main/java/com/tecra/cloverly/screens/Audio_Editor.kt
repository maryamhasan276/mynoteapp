...
import kotlinx.android.synthetic.main.activity_audio__editor.*
import java.io.File
import java.io.IOException

@RequiresApi(api = Build.VERSION_CODES.M)
class Audio_Editor : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onClick(view: View?) {
        when (view!!.id) {
            ...
            R.id.imgBtRecord -> {
                val editText = findViewById<View>(R.id.rec_name) as EditText
                rec = editText.text.toString()
                when{
                    TextUtils.isEmpty(rec) -> Toast.makeText(applicationContext,"Write your title",Toast.LENGTH_SHORT).show()
                    else -> {
                        prepareRecording()
                        startRecording()
                    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_audio__editor)

        val toolbar = findViewById<View>(R.id.toolbr) as Toolbar
    ...
    private fun stopPlaying() {
        ...
        val play = findViewById<View>(R.id.imgViewPlay) as AppCompatImageView
    ...
    private fun startRecording() {
        ...
        val file = File(root.absolutePath + "/Cloverly/Audios")
        if (!file.exists()) {
            file.mkdirs()
        }

        fileName = root.absolutePath + "/Cloverly/Audios/" + ("$rec.mp3")
